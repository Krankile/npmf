...
class TemporalConvNetNoWeightNorm(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2):
        super(TemporalConvNetNoWeightNorm, self).__init__()
        layers = []
        num_levels = len(num_channels)
        for i in range(num_levels):
            dilation_size = 2**i
...
class TcnV4(TcnV1):
    def __init__(
        self, input_size, channels, num_layers, kernel_size, dropout, **kwargs
    ):
        super().__init__(
        ...
        self.tcn = TemporalConvNetNoWeightNorm(
            input_size,
            [channels] * num_layers,
            kernel_size=kernel_size,
            dropout=dropout,
...
class TcnV5(TcnV1):
    def __init__(self, channels, meta_hd, activation, out_len, hd, **kwargs):
        super().__init__(
            channels=channels,
            meta_hd=meta_hd,
            activation=activation,
